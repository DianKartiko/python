// Boiler-specific JavaScript functionality
class BoilerController {
  constructor() {
    this.systemType = "boiler";
    this.temperatureChart = null;
    this.datepickerInstance = null;
    this.deviceIds = ["boiler1", "boiler2"];
    this.chartColors = [
      {
  ...
  connectToDataStream() {
    ...
    eventSource.onerror = (err) => {
      console.error("Boiler stream connection error:", err);
      eventSource.close();
    };
  }
  ...
  async fetchData(selectedDateStr) {
    ...
    try {
      ...
      if (data.length === 0) {
        ...
        noDataMessage.textContent = `Tidak ada data boiler tercatat pada tanggal ${selectedDateStr}.`;
      } else {
        dataSection.style.display = "block";
        tableCaption.textContent = `Menampilkan ${data.length} data boiler untuk ${selectedDateStr}`;

        data.forEach((rowData) => {
          const tr = document.createElement("tr");
          const boiler1 =
            rowData.boiler1 !== null
              ? `${rowData.boiler1.toFixed(1)}°C`
              : "N/A";
          const boiler2 =
            rowData.boiler2 !== null
              ? `${rowData.boiler2.toFixed(1)}°C`
              : "N/A";
          tr.innerHTML = `<td>${rowData.waktu}</td><td>${boiler1}</td><td>${boiler2}</td>`;
  ...
  async renderChart(theme) {
    try {
      ...
      console.error("Gagal merender chart boiler:", error);
    }
  }
...
// Initialize boiler controller when DOM is loaded
document.addEventListener("DOMContentLoaded", () => {
  const boilerController = new BoilerController();
