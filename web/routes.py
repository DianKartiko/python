...
class WebRoutes:
    ...
    def register_routes(self, app):
        ...
        def get_humidity_data():
            """TAMBAHAN: Route untuk get humidity data"""
            selected_date = request.args.get("date")
            system_type = request.args.get("type", "kedi")

            rows = self.db_manager.get_humidity_by_date_pivoted(
                selected_date, latest_only=False, table_type=system_type
            )
        ...
        def get_data_api():
            ...
            rows = self.db_manager.get_data_by_date_pivoted(
                selected_date, latest_only=True, table_type=system_type
            )
        ...
        def get_chart_data():
            try:
                ...
                rows = self.db_manager.get_data_by_date_pivoted(
                    selected_date, table_type=system_type
                )
        ...
        def get_humidity_chart_data():
            try:
                ...
                for row in rows:
                    ...
                    datasets_data.setdefault("Kedi 1 Humidity", []).append(row[1])
                    datasets_data.setdefault("Kedi 2 Humidity", []).append(row[2])
                    datasets_data.setdefault("Kedi 3 Humidity", []).append(row[3])
                    datasets_data.setdefault("Kedi 4 Humidity", []).append(row[4])

                # Color mapping untuk humidity chart
                colors = [
                    ("rgba(102, 16, 242, 1)", "rgba(102, 16, 242, 0.2)"),
                    ("rgba(253, 126, 20, 1)", "rgba(253, 126, 20, 0.2)"),
                    ("rgba(32, 201, 151, 1)", "rgba(32, 201, 151, 0.2)"),
                    ("rgba(232, 62, 140, 1)", "rgba(232, 62, 140, 0.2)"),
                ]
                ...
                try:
                    while True:
                        with self.monitor.data_lock:
                            ...
                            all_humidity = self.monitor.latest_humidity.copy()
                        ...
                        for system_type, devices in all_humidity.items():
                            for device_id, humidity in devices.items():
                                data_payload[f"{device_id}_humidity"] = (
        ...
        def stream_notifications():
            def generate():
                ...
                try:
                    while True:
                        try:
                            notification = self.monitor.notification_queue.get(
                                timeout=25
                            )
                            logger.info(
                                f"[SSE Stream] MENGIRIM NOTIFIKASI KE KLIEN: {notification}"
                            )
        ...
        def download_excel():
            ...
            if system_type == "dryer":
                ws.append(
                    ["Waktu (WIB)", "Dryer 1 (°C)", "Dryer 2 (°C)", "Dryer 3 (°C)"]
                )
            elif system_type == "kedi":
        ...
        def download_combined_excel():
            """Download combined temperature and humidity data"""
            selected_date = request.args.get("date")
            system_type = request.args.get("type", "kedi")

            temp_rows = self.db_manager.get_data_by_date_pivoted(
                selected_date, table_type=system_type
            )
            humidity_rows = self.db_manager.get_humidity_by_date_pivoted(
                selected_date, table_type=system_type
            )
            ...
            # Humidity Sheet
            ws_humidity = wb.create_sheet("Humidity")
            ws_humidity.append(
                ["Waktu (WIB)", "Kedi 1 (%)", "Kedi 2 (%)", "Kedi 3 (%)", "Kedi 4 (%)"]
            )
            ...
            filename = f"laporan_kedi_kombinasi_{selected_date}.xlsx"
            return Response(
                buffer,
                mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                headers={"Content-Disposition": f"attachment;filename={filename}"},
