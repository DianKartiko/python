...
class DatabaseManager:
    ...
    def initialize_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # NEW: Tabel untuk Kedi Humidity
            c.execute(
                """
                CREATE TABLE IF NOT EXISTS kedi_humidity (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    waktu TEXT,
                    kedi_id TEXT, 
                    humidity REAL
    ...
    def insert_temperature(self, waktu, device_id, suhu, table_type="dryer"):
        ...
        table_map = {"dryer": "suhu", "kedi": "kedi_suhu", "boiler": "boiler_suhu"}

        column_map = {"dryer": "dryer_id", "kedi": "kedi_id", "boiler": "boiler_id"}
        ...
        try:
            with self.get_connection() as conn:
                c = conn.cursor()
                c.execute(
                    f"INSERT INTO {table_name} (waktu, {column_name}, suhu) VALUES (?, ?, ?)",
    ...
    # TAMBAHAN: Method untuk insert humidity data
    def insert_humidity(self, waktu, device_id, humidity):
        """Insert data humidity ke database"""
        try:
            with self.get_connection() as conn:
                c = conn.cursor()
                c.execute(
                    "INSERT INTO kedi_humidity (waktu, kedi_id, humidity) VALUES (?, ?, ?)",
                    (waktu, device_id, humidity),
                )
            return True
        except Exception as e:
            logger.error(f"Error inserting humidity data: {e}")
            return False
    ...
    def get_data_by_date_pivoted(self, date_str, latest_only=False, table_type="dryer"):
        ...
        table_name, id_column, device_list = table_map.get(
            table_type, table_map["dryer"]
        )
        ...
        try:
            ...
            for device in device_list:
                case_statements.append(
                    f"MAX(CASE WHEN {id_column} = '{device}' THEN suhu END) as {device}_suhu"
                )
            ...
            logger.error(
                f"Error getting pivoted data for date {date_str} from {table_name}: {e}"
            )
            return []

    # TAMBAHAN: Method untuk get humidity data by date pivoted
    def get_humidity_by_date_pivoted(
        self, date_str, latest_only=False, table_type="kedi"
    ):
        """Mendapatkan data humidity untuk tanggal tertentu dengan pivot"""
        try:
            start_time = f"{date_str} 00:00:00"
            end_time = f"{date_str} 23:59:59"

            device_list = ["kedi1", "kedi2", "kedi3", "kedi4"]

            # Build CASE statements for humidity
            case_statements = []
            for device in device_list:
                case_statements.append(
                    f"MAX(CASE WHEN kedi_id = '{device}' THEN humidity END) as {device}_humidity"
                )
            ...
            logger.error(
                f"Error getting humidity pivoted data for date {date_str}: {e}"
            )
            return []
    ...
    def get_data_since(self, since_time, table_type="dryer"):
        ...
        try:
            with self.get_connection() as conn:
                c = conn.cursor()
                c.execute(
                    f"SELECT * FROM {table_name} WHERE datetime(waktu) >= datetime(?) ORDER BY waktu",
                    (since_time,),
                )
                return c.fetchall()
        except Exception as e:
            logger.error(
                f"Error getting data since {since_time} from {table_name}: {e}"
            )
            return []
    ...
    def get_recent_data(self, limit=5, table_type="dryer"):
        ...
        try:
            with self.get_connection() as conn:
                c = conn.cursor()
                c.execute(
                    f"SELECT waktu, {id_column}, suhu FROM {table_name} ORDER BY id DESC LIMIT ?",
                    (limit,),
                )
                return c.fetchall()
        except Exception as e:
