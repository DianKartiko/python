...
class MQTTService:
    ...
    def _on_connect(self, client, userdata, flags, rc):
        """Callback ketika terhubung ke MQTT broker - UPDATE untuk humidity topics"""
        if rc == 0:
            ...
            # Subscribe to temperature topics (existing)
            for topic_name, topic in self.config.MQTT_TOPICS.items():
                if topic:
                    self.client.subscribe(topic)
                    logger.info(f"Subscribed to {topic_name}: {topic}")

            # TAMBAHAN: Subscribe to humidity topics (jika ada)
            if hasattr(self.config, "MQTT_HUMIDITY_TOPICS"):
                for topic_name, topic in self.config.MQTT_HUMIDITY_TOPICS.items():
                    if topic:
                        self.client.subscribe(topic)
                        logger.info(f"Subscribed to {topic_name}: {topic}")
        else:
    ...
    def _on_message(self, client, userdata, msg):
        """Callback ketika menerima message MQTT - Kembali ke original"""
        try:
            value = float(msg.payload.decode())
            if self.data_callback:
                self.data_callback(value, msg.topic)
